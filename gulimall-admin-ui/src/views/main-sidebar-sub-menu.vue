<template>
  <el-submenu v-if="menu.children && menu.children.length >= 1" :index="String(menu.menuId)" :popper-append-to-body="false">
    <template slot="title">
  ...
  <el-menu-item v-else :index="String(menu.menuId)" @click="gotoRouteHandle(menu.menuId)">
    <svg class="icon-svg aui-sidebar__menu-icon" aria-hidden="true"><use :xlink:href="`#${menu.icon}`"></use></svg>
    <span>{{ menu.name }}</span>
  </el-menu-item>
</template>
