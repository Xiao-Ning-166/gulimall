<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="id" label="采购单编号" align="center" />
      <el-table-column prop="name" label="采购单名称" align="center" />
      <el-table-column prop="assigneeName" label="采购人姓名" align="center" />
      <el-table-column prop="phone" label="采购人手机号" align="center" />
      <el-table-column prop="priority" label="优先级" align="center" />
      <el-table-column label="状态" align="center">
        <template slot-scope="{row}">
          <el-tag v-show="row.status === 0">新建</el-tag>
          <el-tag v-show="row.status === 1" type="info">已分配</el-tag>
          <el-tag v-show="row.status === 2" type="warning">已领取</el-tag>
          <el-tag v-show="row.status === 3" type="success">已完成</el-tag>
          <el-tag v-show="row.status === 4" type="danger">有异常</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="orderFormTitle" :visible.sync="orderFormVisible">
      ...
      >
        <el-form-item label="采购单名称" prop="name">
          <el-input v-model="orderForm.name" placeholder="请输入采购单名称" />
        </el-form-item>
        <el-form-item label="优先级" prop="priority">
          <el-input-number v-model="orderForm.priority" />
        </el-form-item>
        <el-form-item label="仓库名称" prop="wareId">
          <storage-select class="filter-item" :storage-id.sync="orderForm.wareId" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="orderFormTitle==='新增'?createData():updateData()">
          确认
        </el-button>
      </div>
    </el-dialog>
...
} from '@/api/storage/purchaseOrder.js'
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验规则
      rules: {
        name: [{ required: true, message: '请输入采购采购单名称', trigger: 'blur' }],
  ...
  methods: {
    ...
    createData: debounce(function() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          // 新增
          addPurchaseOrder(this.orderForm).then((res) => {
            if (res.success) {
    ...
    updateData: debounce(function() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          // 修改
          updatePurchaseOrderById(this.orderForm).then((res) => {
            if (res.success) {
              this.orderFormVisible = false
              // 重新加载数据
              this.loadData()
    ...
    // 调用删除方法
    deleteById(ids) {
      deletePurchaseOrderById(ids).then((res) => {
        if (res.success) {
          // 重新加载数据
          this.loadData()
          this.$notify({
